...
function AddUser() {
  ...
  return (
    <Container maxWidth="lg" sx={{ height: '100vh', paddingTop: '36px' }}>
      ...
      <Paper elevation={3} sx={{ height: 'calc(100% - 125px)', padding: '12px 24px' }}>
        <FormControl size="medium" sx={{ width: '300px' }}>
          ...
          >
            {countriesData.map((country: ICountry) => <MenuItem key={country.code} value={country.code}>{country.name}</MenuItem>)}
          </Select>
        </FormControl>
        <br />
